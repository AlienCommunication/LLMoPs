...
def build_recent_window(
    ...
    os.makedirs(os.path.dirname(recent_stats_path), exist_ok=True)
    stats = compute_stats(recent_jsonl_path)

    with open(recent_stats_path, "w", encoding="utf-8") as f:
        json.dump(stats, f, ensure_ascii=True, indent=2)

    return json.dumps({"recent_status": "ok", "n": stats["n"]})
